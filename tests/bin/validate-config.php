...
$universal_geo_test_config_file = $argv[1] ?? '';

if ( ! file_exists( $universal_geo_test_config_file ) ) {
	fprintf( STDERR, "Error: Config file not found: %s\n", $universal_geo_test_config_file );
	exit( 1 );
}

// Check for $table_prefix in the config file source without executing it.
$universal_geo_test_config_content = file_get_contents( $universal_geo_test_config_file );

if ( ! preg_match( '/\$table_prefix\s*=/', $universal_geo_test_config_content ) ) {
