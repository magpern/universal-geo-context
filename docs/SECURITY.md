...
**Status: M3 complete.** The trust-boundary threats (spoofing, header
precedence, provider hardening) and the MaxMind file-read threat below are
implemented and tested. The remote-provider row remains M4.
...
| **DoS via provider latency** | Local providers (`CloudflareHeaderProvider`, `WooCommerceProvider`, `MaxMindProvider`) perform no network I/O; `MaxMindProvider` additionally opens its `Reader` at most once per request (memoized, including a failed open, so a corrupt database costs one attempt, not one per lookup). M4's remote provider will add timeouts, circuit breaking, and negative caching. |
